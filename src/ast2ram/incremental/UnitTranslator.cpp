...
std::vector<const ast::Relation*> UnitTranslator::positiveDeleteDeps(const ast::Relation& rel) const {
    std::set<std::string> seen;
    std::vector<const ast::Relation*> deps;
    for (auto&& clause : context->getProgram()->getClauses(rel)) {
        if (isA<ast::SubsumptiveClause>(clause) || context->isRecursiveClause(clause)) {
            continue;
        }
        for (const auto* lit : clause->getBodyLiterals()) {
            const auto* atom = as<ast::Atom>(lit);  // a positive body literal; negated atoms are ast::Negation
            if (atom == nullptr) {
                continue;
            }
            const ast::Relation* d = context->getProgram()->getRelation(*atom);
            if (d != nullptr && seen.insert(getConcreteRelationName(d->getQualifiedName())).second) {
                deps.push_back(d);
            }
        }
    }
...
Own<ram::Statement> UnitTranslator::generateSetDifference(const std::string& src, const std::string& other,
        const std::string& dest, std::size_t arity) const {
    Own<ram::Condition> notInOther;
    if (arity == 0) {
        // Nullary: data identity is mere presence — "t.data not in other" is "other is empty".
        notInOther = mk<ram::EmptinessCheck>(other);
    } else {
        VecOwn<ram::Expression> key;
        for (std::size_t i = 0; i < arity; i++) {
            key.push_back(mk<ram::TupleElement>(0, i));
        }
        key.push_back(mk<ram::UndefValue>());  // @iteration: match on data columns only
        notInOther = mk<ram::Negation>(mk<ram::ExistenceCheck>(other, std::move(key)));
    }
    VecOwn<ram::Expression> copy;
    for (std::size_t i = 0; i < arity + 1; i++) {
        copy.push_back(mk<ram::TupleElement>(0, i));  // all columns, including @iteration
    }
    return mk<ram::Query>(mk<ram::Scan>(src, 0,
            mk<ram::Filter>(std::move(notInOther), mk<ram::Insert>(dest, std::move(copy)))));
}
...
Own<ram::Statement> UnitTranslator::generateIncrementalDelete(const ast::Relation& rel) const {
    ...
    if (rel.getArity() == 0) {
        ...
        // Over-delete candidates, CORRECT for SIMULTANEOUS multi-atom deletion via MERGE-BACK. The per-atom delta
        // rules (generateDeltaRules over diff_minus) catch one deleted atom while reading the OTHERS over their
        // already-updated (new) state, so they miss a head losing two body facts at once. Fix: temporarily
        // restore each positive dependency to its OLD state (current ∪ truly-deleted) before the per-atom rules,
        // so non-target atoms read old — the union over the k versions is then the join over the OLD database
        // (the exact deletion delta). Equivalent to a 2^k-1 subset enumeration but only k versions (no codegen
        // blow-up). A dependency of a delta-eligible stratum is itself eligible (delta) or EDB, so its @swap_d
        // scratch is free to borrow here.
        auto deps = positiveDeleteDeps(rel);
        for (const ast::Relation* d : deps) {
            // Stage d's TRULY-deleted tuples (diff_minus_d \ d — survivors re-derived by d's own delete are
            // already back in d) into @swap_d, then merge them into d to reconstruct d^old.
            const std::string dMain = getConcreteRelationName(d->getQualifiedName());
            appendStmt(result, mk<ram::Clear>(swapName(d)));
            appendStmt(result, generateSetDifference(diffMinusName(d), dMain, swapName(d), d->getArity()));
            appendStmt(result, generateMergeRelations(d, dMain, swapName(d)));
        }
        appendStmt(result, generateDeltaRules(rel, "diff_minus_", "diff_minus_", /* includeRecursive */ false));
        for (const ast::Relation* d : deps) {
            // Restore d to its NEW state by erasing exactly the staged tuples; clear the borrowed scratch.
            appendStmt(result, generateEraseAll(d, getConcreteRelationName(d->getQualifiedName()), swapName(d)));
            appendStmt(result, mk<ram::Clear>(swapName(d)));
        }
        // Negation sign-flips: a negated atom that just became TRUE also over-deletes (its candidates join the
        // positive ones in diff_minus_<H> before the erase). Runs after the restore, so it reads CURRENT state.
        // A no-op for negation-free relations.
...
std::set<std::size_t> UnitTranslator::computeDeltaEligible(
    ...
    // aggregate. Negation IS supported (the negation-delta rules seed the three-term update from sign flips);
    // aggregates still force a recompute (they must see the whole relation). The over-delete handles any body
    // size via merge-back (k versions, no enumeration blow-up), so there is no body-size cap. EDB strata have no
